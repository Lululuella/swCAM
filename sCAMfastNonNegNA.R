sCAMfastNonNegNA <- function(X, A, Sest, iter=1, r = 1, lambda = 0, eta = 100, 
    ...
    while(iter0 < iter){
        ...
        while((epPri > eps || epDual > eps) && j < iteradmm){
            ...
            for (ik in seq_len(K)) {
                U[1:L + (ik-1)*L,][naMask] <- H[1:L + (ik-1)*L,][naMask] / 2
            }
    ...
    return(list(A=A, S=Si, W=W, Resi=Resi, epPri=epPri, epDual=epDual, iterrun = iterrun, nuNorm=nuNorm))
}
