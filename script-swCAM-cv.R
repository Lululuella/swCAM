# K-fold cross validation to select eta parameter
...
#etaAll: candidate eta parameter
#iteradmm: max iteration number; larger value will cause longer running time 
Xn <-
Aest <-
Sest <-
etaAll <- c(10000, 5000, 2000, 1500, 1000, 800, 500, 400, 300, 200, 150, 100, 80, 50, 40, 30, 20, 10 , 8, 5 , 2, 1, 0.5, 0.2, 0.1, 0.05, 0.01, 0.001, 0.0001, 0.00001)
iteradmm <- 1000
Kfold <- 10 
...
res <- foreach (ifold = 1:Kfold, .combine=rbind) %dopar% {
    ...
    for (eta in etaAll) {
        rsCAMdtrain <- sCAMfastNonNegNA(Xtrain, Aest, Sest, iter = 1,  r = 1, lambda = 0, eta = eta, 
        ...
        erreta <- c(erreta, sum((Xest[sample_exp == ifold] - Xn[sample_exp == ifold])^2),
...
df.cv <- data.frame(x=factor(rep(etaAll, each=nrow(rmse))),y=c(rmse))


library(ggplot2)
library(scales)
pict <- ggplot(subset(df.cv, x %in% etaAll), aes(x=x, y=y)) + 
    geom_boxplot() + 
